...
class UpInfoSpider:
    ...
    async def get_video_nums(self, mid: int):
    ...
    async def get_up_info(self, mid: int) -> Optional[UpInfo]:
        ...
        if not relation:
            statistics.record_failed_mid(mid)
            return None
        ...
        statistics.record_accept_mid(mid)
    ...
    async def process_up_info(self, mid: int, save_path: str):
        try:
            if info := await self.get_up_info(mid):
                await storage.write(info.to_json(ensure_ascii=False), save_path)
        except Exception as e:
            statistics.record_failed_mid(mid)
            logger.exception(e)
    
    async def run_with_range(self, start:int, end:int):
        await self._client.init()
        # create 1w coroutine on Intel(R) Xeon(R) Gold 6148 CPU @ 2.40GHz
        # costs 40MB, cpu 91%
        save_path = f"data/up_info/{start}_{end}.dat"
        parallel_task_nums = 5000
        for i in range(start, end, parallel_task_nums):
            tasks = (self.process_up_info(mid, save_path) for mid in range(i, i + parallel_task_nums))
